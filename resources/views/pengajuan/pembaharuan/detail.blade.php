...
@section('content')
        ...
        <div
            ...
            <div id="navMenus" class="tw-flex tw-gap-4 tw-w-full tw-flex-col tw-p-4">
                <form class="tw-flex tw-flex-col tw-gap-7 tw-items-end" method="POST"
                    action="{{ route('pengajuan.confirm.pembaharuan') }}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
    ...
    <div id="modalReject"
        class="tw-hidden tw-px-5 md:tw-px-0 tw-animate-fade tw-animate-duration-300 tw-animate-ease-in-out modal-menu tw-z-40 tw-fixed insert-0 tw-bg-n1000 tw-bg-opacity-20 tw-overflow-y-auto tw-h-full tw-w-full ">
        <div
            class="tw-w-mobile-full-w md:tw-w-2/4 tw-relative tw-top-1/2 tw-left-1/2 -tw-translate-x-1/2 -tw-translate-y-1/2 tw-bg-n100 tw-rounded-md tw-overflow-hidden tw-border-[1px] ">
            <div class="tw-flex tw-items-center tw-px-4 tw-h-14 tw-border-b-[1px]">
            ...
            <div id="navMenus" class="tw-flex tw-gap-4 tw-w-full tw-flex-col tw-p-4">
                <form class="tw-flex tw-flex-col tw-gap-7 tw-items-end" action="{{ route('pengajuan.reject.pembaharuan') }}"
                    method="POST">
                    @method('PUT')
                    @csrf
                    ...
                    <div class="tw-flex tw-flex-col tw-gap-2 tw-pt-6">
                        ...
                        <div class="tw-flex tw-flex-col tw-gap-3">
                            ...
                            @include('components.form.textdetail', [
                                ...
                                'content' =>
                                    'Rp ' . number_format($pengajuan->keluarga->tagihan_listrik, 0, ',', '.'),
                            ])
                            @include('components.form.textdetail', [
                                'title' => 'Luas Bangunan',
